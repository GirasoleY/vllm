...
def test_autoregressive_decode_forwards_cpu_bounds_and_draft_step():
    """The propose-to-decode path must supply both new metadata arguments."""
    num_reqs = 2
    ...
    input_batch = SimpleNamespace(
        ...
        num_tokens=num_reqs,
        num_reqs=num_reqs,
        num_scheduled_tokens=torch.ones(num_reqs, dtype=torch.int32),
        seq_lens_cpu_upper_bound=seq_lens_cpu_upper_bound,
        seq_lens=torch.tensor([40, 56], dtype=torch.int32),
    ...
    fake = SimpleNamespace(
        ...
        last_token_indices=torch.zeros(num_reqs, dtype=torch.int64),
        ...
        _prepare_eplb_forward=MagicMock(),
        on_prefill_begin=MagicMock(),
        on_prefill_end=MagicMock(),
        on_multi_step_decode_begin=MagicMock(),
        on_multi_step_decode_end=MagicMock(),
        _prefill=MagicMock(),
        draft_tokens=torch.zeros(num_reqs, num_speculative_steps, dtype=torch.int64),
        advance_draft_positions=True,
        use_fused_multi_step_decode=False,
        idx_mapping=torch.arange(num_reqs, dtype=torch.int32),
    ...
    fake._multi_step_decode = MethodType(
        AutoRegressiveSpeculator._multi_step_decode, fake
    )
    ...
    with (
        patch.object(autoregressive_speculator, "prepare_prefill_inputs"),
        patch.object(autoregressive_speculator, "prepare_decode_inputs"),
        patch.object(
            autoregressive_speculator,
            "dispatch_cg_and_sync_dp",
            side_effect=[(batch_desc, None), (batch_desc, None)],
        ),
        patch.object(
            autoregressive_speculator,
        ...
        AutoRegressiveSpeculator.propose(
            ...
            attn_metadata={},
            slot_mappings={},
            last_hidden_states=torch.zeros(num_reqs, 4),
