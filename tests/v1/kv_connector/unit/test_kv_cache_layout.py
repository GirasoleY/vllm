...
def test_stride_aware_mla_backends_opt_into_cross_layer_kv_cache():
    ...
    for backend in (FlashMLABackend, FlashAttnMLABackend, FlashInferMLABackend):
...
def test_unverified_mla_backends_remain_opted_out():
    """Backends without verified stride-aware decode kernels keep the
    identity default from MLACommonBackend."""
    ...
    for backend in (CutlassMLABackend, TritonMLABackend):
        stride_order = backend.get_kv_cache_stride_order(
            include_num_layers_dimension=True
        )
        assert stride_order == (0, 1, 2, 3), backend.get_name()
