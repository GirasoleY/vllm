...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Token-interleaved equal-DCP caches compact each rank's cyclic token
        # stream into contiguous local physical blocks. The same-rank page
        # suffix therefore transfers directly even when one logical page spans
        # many kernel blocks (Kimi-K3 align-Mamba geometry).
        pytest.param(
            [100],
            [200, 201, 202],
            3,
            ...
            48,
            48,
            list(range(100 * 48, 101 * 48)),
            list(range(202 * 48, 203 * 48)),
            id="equal_dcp_token_interleave_large_logical_page",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(8, 1, 8, 1, False, id="equal_dcp_token_interleave"),
        pytest.param(8, 1, 8, 16, True, id="equal_dcp_layout_mismatch"),
        pytest.param(8, 16, 1, 1, False, id="asymmetric_whole_page_source"),
        pytest.param(8, 1, 1, 1, True, id="asymmetric_token_interleave_source"),
        pytest.param(1, 1, 8, 16, False, id="asymmetric_whole_page_remote"),
        pytest.param(1, 1, 8, 1, True, id="asymmetric_token_interleave_remote"),
    ],
)
def test_validate_dcp_interleave_compatibility(
    local_dcp_size,
    local_interleave,
    remote_dcp_size,
    remote_interleave,
    ...
    worker.cp_kv_cache_interleave_size = local_interleave
    worker.block_size = 16
    worker._physical_blocks_per_logical_kv_block = 1

    call = lambda: worker._validate_dcp_interleave_compatibility(  # noqa: E731
        remote_dcp_size=remote_dcp_size,
        remote_interleave=remote_interleave,
        remote_block_size=16,
        remote_physical_per_logical=1,
        remote_engine_id="remote",
    )
    if should_raise:
        with pytest.raises(RuntimeError, match="DCP KV transfer"):
            call()
    else:
        call()
