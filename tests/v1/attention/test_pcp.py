...
def test_fused_workspace_gathers_request_sharded_decode_rows():
    resolve = pcp.DirectPCPFusedNormRopeWorkspace._resolve_replicated_decode_tokens

    assert resolve(7, shard_decode_requests=False) == 7
    assert resolve(7, shard_decode_requests=True) == 0
