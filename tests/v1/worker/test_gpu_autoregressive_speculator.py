...
from vllm.v1.worker.gpu.spec_decode.mtp import speculator as mtp_spec_module
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"tensor_parallel_size": 2},
            "PCP speculative decoding does not support DCP",
        ),
        (
            {
                "tensor_parallel_size": 2,
...
@pytest.mark.parametrize(("sharded_pcp", "expected"), [(False, True), (True, False)])
def test_mtp_topk_index_sharing_is_disabled_for_sharded_pcp(
    monkeypatch, sharded_pcp, expected
):
    inner_model = SimpleNamespace(
        set_skip_topk=Mock(),
        compact_topk_indices=Mock(),
    )
    draft_model = SimpleNamespace(model=inner_model)
    ...
    speculator.sharded_pcp = sharded_pcp
    monkeypatch.setattr(
        mtp_spec_module, "load_eagle_model", Mock(return_value=draft_model)
    )

    assert speculator.load_draft_model(None, set()) is draft_model
    assert speculator.share_mtp_topk_indices is expected
...
@pytest.mark.parametrize(
    "decode_method", ["_multi_step_decode", "_fused_multi_step_decode"]
)
def test_sharded_continuation_metadata_is_decode_only(monkeypatch, decode_method):
    speculator = object.__new__(_TestSpeculator)
    ...
    getattr(speculator, decode_method)(
        num_reqs=2,
        skip_attn=False,
