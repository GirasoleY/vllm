...
class TestDirectDCPGating:
    ...
    def test_kv_final_layout_returns_contiguous_planes(self, monkeypatch):
        ...
        workspace.token_dim = 16
        workspace.plane_split_dim = 8
        workspace.world_size = 2
        workspace.rank = 0
        workspace.multicast_ptrs = [(1, 2)]
        workspace.received_kv = torch.arange(256, dtype=torch.bfloat16).view(
            1, 2, 8, 16
        )
        ...
        assert kv_c.shape == (4, 1, 8)
        assert k_pe.shape == (4, 1, 8)
        assert kv_c.is_contiguous()
        assert k_pe.is_contiguous()
        assert kv_c.flatten().tolist() == list(range(32))
        assert k_pe.flatten().tolist() == list(range(64, 96))
        direct_op.assert_called_once()

        with pytest.raises(ValueError, match="configured layout"):
            workspace.gather(
                torch.empty(4, 16, dtype=torch.bfloat16),
                torch.arange(4, dtype=torch.int32),
                output_tokens=4,
...
def test_mla_dcp_manager_falls_back_when_either_kv_plane_is_unaligned(monkeypatch):
    import vllm.v1.attention.ops.dcp_utils as dcp_manager
    ...
    direct_factory = MagicMock(return_value=None)
    monkeypatch.setattr(
        dcp_manager, "get_direct_dcp_kv_gather_workspace", direct_factory
    )
    manager = dcp_manager.MLADCPManager(
        vllm_config=_manager_config(),
        device=torch.device("cpu"),
    ...
    assert not manager.init_kv_gather(64, 16, 4, torch.bfloat16)
    assert not manager.use_direct_kv_gather
    assert isinstance(manager._kv_gather, functools.partial)
...
def _distributed_direct_kv_gather_worker(env: dict[str, str]) -> None:
    ...
    try:
        ...
        for dtype_name in ("bfloat16", "float16", "float8_e4m3fn"):
            dtype = _dtype_from_name(dtype_name)
            workspace = dcp_utils.DirectDCPKVGatherWorkspace(
                dist.group.WORLD,
                device,
            ...
            for replay, layout in enumerate((alternate_layout, full_layout)):
                maps, output_tokens = layout_maps(layout)
                assert output_tokens == capture_output_tokens
                captured_input.copy_(
                    make_local(rank, 22 + replay, len(maps[rank]), dtype)
                )
                ...
                graph.replay()
                torch.accelerator.synchronize()
                expected_kv_c, expected_k_pe = expected_planes(
                    layout, 22 + replay, dtype
                )
                assert torch.equal(
                    captured_kv_c.view(torch.uint8), expected_kv_c.view(torch.uint8)
                )
                ...
                assert int(graph_workspace.epoch[0].item()) == int(
                    epochs_before[0].item()
                )
