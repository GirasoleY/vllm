...
def test_cp_gather_cache_mla_with_seq_starts(
    ...
    for req_id in range(batch_size):
        block_table[req_id] = torch.randperm(num_blocks, device=device)

    expected_rows = []
    for req_id in range(batch_size):
        start = seq_starts[req_id].item()
        length = seq_lens[req_id].item()
        for source_token in range(start, start + length):
...
def test_cp_gather_cache_mla_large_uneven_sequences(
    ...
    seq_starts_tensor = torch.tensor([3, 17, 5, 31], dtype=torch.int32, device=device)
    seq_starts = seq_starts_tensor if use_seq_starts else None
    seq_lens = torch.tensor([0, 17, 32680, 71], dtype=torch.int32, device=device)
    batch_size = seq_lens.shape[0]
    cu_seq_lens = torch.zeros(batch_size + 1, dtype=torch.int32, device=device)
    cu_seq_lens[1:] = seq_lens.cumsum(dim=0)
    ...
    expected_batches = []
    for req_id in range(batch_size):
        start = seq_starts_tensor[req_id].item() if use_seq_starts else 0
        length = seq_lens[req_id].item()
        if length == 0:
            continue
        source_tokens = torch.arange(start, start + length, device=device)
        logical_blocks = source_tokens // block_size
        physical_blocks = block_table[req_id, logical_blocks].long()
