...
def test_dspark_context_precompute_uses_single_tensor_rope(
    ...
    model.context_kv_proj = lambda states: torch.arange(
        states.shape[0] * 4, dtype=states.dtype
    ).view(states.shape[0], 4)
    ...
    monkeypatch.setattr(
        dspark_mla, "current_workspace_manager", lambda: FakeWorkspaceManager()
    )
    monkeypatch.setattr(
        dspark_mla.ops,
        "rms_norm",
