...
- CLI and YAML mappings for `draft_parallel_config` accept only
  `tensor_parallel_size`, `prefill_context_parallel_size`, and
  `decode_context_parallel_size`. Python callers pass a complete
  `ParallelConfig`. Other fields are not user-overridable; the runtime derives
  them from the target topology and selected draft layout.
- On a PCP target, omitting `draft_parallel_config` preserves the existing
  replicated-draft default (draft PCP=1). An explicit partial mapping inherits
