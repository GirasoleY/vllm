...
class DirectPCPFusedNormRopeWorkspace(DirectCPWorkspace):
    ...
    Each rank contributes the same number of local tokens. Decode rows may be
    replicated or request-sharded, while prefill rows are sequence-sharded. The
    dispatch kernel writes cache-ready payloads into the symmetric window, and
    the combine kernel scatters the unique rows into local paged caches.
    """
    ...
    @staticmethod
    def _resolve_replicated_decode_tokens(
        num_decode_tokens: int,
        shard_decode_requests: bool,
    ) -> int:
        # The combine kernel de-duplicates this prefix by reading it from rank
        # 0. Request-sharded decode rows are unique, so gather the whole local
        # payload from every rank just like prefill rows.
        return 0 if shard_decode_requests else num_decode_tokens
    ...
    def forward(
        ...
        *,
        ...
        num_replicated_decode_tokens = self._resolve_replicated_decode_tokens(
            num_decode_tokens,
