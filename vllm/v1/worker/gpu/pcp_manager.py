...
class PCPManager:
    ...
    def __init__(
        ...
        # Persistent index buffers: the KDA gather/scatter runs inside the
        # model forward, so the index tensors must keep stable addresses for
        # cudagraph replay while their contents are re-staged every step.
        self._hidden_restore_idx_buf: torch.Tensor | None = None
        self._padded_gather_idx_buf: torch.Tensor | None = None
        if max_num_tokens is not None:
            self._hidden_restore_idx_buf = torch.empty(
                max_num_tokens, dtype=torch.int64, device=device
            )
            self._padded_gather_idx_buf = torch.empty(
                max_num_tokens * pcp_world_size, dtype=torch.int64, device=device
            )
    ...
    def _build_batch_layout(
        ...
        assert self._hidden_restore_idx_buf is not None
        assert self._padded_gather_idx_buf is not None
        self._hidden_restore_idx = async_copy_to_gpu(
            hidden_restore_idx,
            out=self._hidden_restore_idx_buf[: len(hidden_restore_idx)],
        )
        self._padded_gather_idx = async_copy_to_gpu(
            padded_gather_idx,
            out=self._padded_gather_idx_buf[: len(padded_gather_idx)],
        )
    ...
    def partition_batch(
        ...
        if num_local_tokens > 0:
            ...
            seq_lens[:num_local_reqs].copy_(local_end_positions + 1)
