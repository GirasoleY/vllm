...
class PCPManager:
    ...
    def partition_batch(
        ...
        # Reuse the GPU cursor materialized by prepare_inputs(). The CPU
        # num_computed_tokens cursor may still be optimistic after rejection.
        torch.index_select(
        ...
        # The global batch already contains the sampled and draft tokens.
        # Recombining the local view would overwrite its final draft token.
    ...
    def restore_hidden_states(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        padded = restored.new_zeros((num_tokens_padded, *restored.shape[1:]))
        padded[:num_tokens].copy_(restored)
        return padded

    def restore_hidden_state_buffer(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """Restore a persistent max-token-sized target hidden-state buffer."""
        assert self._padded_gather_idx is not None
