...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def propose(
        ...
        pcp_local_batch = (
            ...
            if self.sharded_pcp and self.pcp_manager is not None
            else None
        )
        model_input_batch = (
            pcp_local_batch if pcp_local_batch is not None else input_batch
        )
        model_num_tokens_padded = model_input_batch.num_tokens_after_padding
        self.hidden_states[:model_num_tokens_padded].copy_(hidden_states)
        ...
        uniform_token_count = get_uniform_decode_token_count(
            model_input_batch.num_reqs,
            # Use the actual number of tokens without padding added by
            # the target model during FULL cudagraph.
            model_input_batch.num_tokens,
            model_input_batch.num_scheduled_tokens.max(),
        ...
        self._prepare_eplb_forward(model_input_batch.num_tokens)
        ...
        if pcp_local_batch is not None and not (dummy_run and skip_attn_for_dummy_run):
            # The target used rank-local scratch block tables. Continuation
            # uses the drafter's global request batch on every PCP rank.
            self.block_tables.gather_block_tables(
                input_batch.idx_mapping,
