...
class MooncakeStoreCoordinator:
    ...
    def _exact_partial_hit_key_exists(
        ...
        """Whether the reconciled FullAttention boundary can be loaded."""
        if not self.enable_partial_hash_hits or not self.attention_groups:
            return True

        first_group = self.attention_groups[0]
        if not isinstance(first_group.spec, FullAttentionSpec):
            return True
        hash_idx = hit_length // self.hash_block_size - 1
        return (
            cached_block_pool.get_cached_block(
                block_hashes[hash_idx], first_group.group_ids
            )
            is not None
        )
...
def is_simple_full_mamba_hybrid(
    ...
    """Whether groups collapse to one FullAttention and one align-Mamba spec."""
    attention_specs: list[KVCacheSpec] = []
    for group in kv_cache_groups:
        spec = _unwrap_spec(group.kv_cache_spec)
        if spec not in attention_specs:
            attention_specs.append(spec)
    if len(attention_specs) != 2:
        return False
    full_specs = [
        spec for spec in attention_specs if isinstance(spec, FullAttentionSpec)
    ]
    mamba_specs = [spec for spec in attention_specs if isinstance(spec, MambaSpec)]
    return (
        len(full_specs) == 1
