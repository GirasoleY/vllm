...
class SpeculativeConfig:
    ...
    """Users should pass ``draft_tensor_parallel_size`` or
    ``draft_parallel_config``. This field only provides a tailored diagnostic
    when ``tensor_parallel_size`` is placed at the wrong nesting level."""
    ...
    """TP/PCP/DCP policy for an integrated draft model. CLI and YAML mappings
    are treated as partial overrides of the target topology; Python callers
    pass a ``ParallelConfig``. Supported layouts depend on the implementation.
    When omitted, vLLM derives the existing default."""
    _draft_parallel_config_provenance: tuple[bool, int, int, int] | None = field(
        default=None,
        compare=False,
        repr=False,
        kw_only=True,
    )
    """Internal origin and policy snapshot used to make config replacement
    idempotent after ``draft_parallel_config`` has been materialized."""
    ...
    def _validate_draft_parallel_config_input(cls, data: Any) -> Any:
        if isinstance(data, ArgsKwargs):
            init_fields = tuple(
                ...
                and not parallel_field.kw_only
            )
        ...
        provenance = values.get("_draft_parallel_config_provenance")
        if (
            isinstance(draft, ParallelConfig)
            and isinstance(target, ParallelConfig)
    ...
    def __post_init__(self):
        input_draft_parallel_config = self.draft_parallel_config
        provenance = self._draft_parallel_config_provenance
        input_policy = (
            (
                input_draft_parallel_config.tensor_parallel_size,
                input_draft_parallel_config.prefill_context_parallel_size,
                input_draft_parallel_config.decode_context_parallel_size,
            )
            if input_draft_parallel_config is not None
            else None
        )
        draft_parallel_config_was_provided = input_policy is not None and (
            provenance is None or provenance[0] or input_policy != provenance[1:]
        )
        user_draft_parallel_config = (
            input_draft_parallel_config if draft_parallel_config_was_provided else None
        )
        if (
        ...
        effective_draft_parallel_config = self.draft_parallel_config
        if effective_draft_parallel_config is None:
            self._draft_parallel_config_provenance = None
        else:
            self._draft_parallel_config_provenance = (
                draft_parallel_config_was_provided,
                effective_draft_parallel_config.tensor_parallel_size,
                effective_draft_parallel_config.prefill_context_parallel_size,
                effective_draft_parallel_config.decode_context_parallel_size,
            )
    ...
    def _verify_args(self) -> Self:
        if self.tensor_parallel_size is not None:
            raise ValueError(
                ...
                "speculative_config. Please pass 'draft_tensor_parallel_size' "
                "or 'draft_parallel_config' instead."
            )
