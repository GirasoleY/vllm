...
class Glm5NextLinearAttention(GatedDeltaNetAttention):
    ...
    def _forward_kcp(
        ...
        the conv halo tails cross ranks. Non-KCP rows (plain decodes, spec
        verifies, single-token extends) live on a single owner rank, so they
        are gathered and computed redundantly on every rank to keep the
        replicated mamba caches coherent.
        """
