...
Planning, communication and state merging live in ``ops/kcp.py``. This module
is imported lazily by the KCP path so ordinary KDA does not require CuTe DSL.
"""
...
def prepare_kcp(
    ...
    *,
    ...
    exactly once and use contiguous int32 or int64 tensors; KcpPlan constructs
    them on the CPU. Empty segments need no chunk entry. The returned A
    contains pre-inverted 16x16 diagonal blocks for the triangular merge.
    """
