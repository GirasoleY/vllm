...
class KcpPlan:
    ...
    halo_tail_idx: torch.Tensor  # [N_loc, 3] int64, slot-ordered tails; -1 = prefix
...
def _simulate_rank_segments(
    ...
    """Replicate PCPManager's local partition: (req, abs_start, len) rows in
    local batch order (pure prefills moved last, stable)."""
    num_chunks = 2 * world
    segments: list[tuple[int, int, int]] = []
    decode_ordinal = 0
    for req, num_tokens in enumerate(q_g):
        ...
        if bool(is_pref[req]):
            ...
            # MRV2 PCP runs with decode_context_parallel_size == 1, where each
            # decode request has a single owner rank.
            chunk_size = query_len
            owner = decode_ordinal % world
            decode_ordinal += 1
            chunk_indices = (0,) if rank == owner else ()
        for chunk_idx in chunk_indices:
            chunk_offset = chunk_idx * chunk_size
            chunk_len = min(chunk_size, query_len - chunk_offset)
            if chunk_len > 0:
                segments.append((req, int(start0[req]) + chunk_offset, chunk_len))
    segments.sort(key=lambda seg: bool(is_pref[seg[0]]) and seg[1] == 0)
    return segments
...
def build_kcp_plan(mgr, device: torch.device) -> KcpPlan | None:
    ...
    rank = mgr.pcp_rank
    if world <= 1 or not mgr.shard_decode_requests:
        return None
    ...
    # Single-token rows (plain decodes, spec verifies and 1-token extends) stay
    # on the redundant path: they live on a single rank, so KCP-local compute
    # would desynchronize the replicated caches.
    kcp_pref = is_pref & (q_g > 1)
    ...
    sim = _simulate_rank_segments(q_g, is_pref, start0, world, rank)
    sim_tokens = sorted(
    ...
    for r_ in range(world):
        ...
        for g, a, length, _tok in _segments_with_offsets(
            _simulate_rank_segments(q_g, is_pref, start0, world, r_)
        ):
            if kcp_pref[g]:
                continue
    ...
    for row_i, (n, c, t0, qlen) in enumerate(scan_rows):
        ...
        window: list[tuple[int, int]] = [(-1, -1)] * 3
        for c_ in range(c):
    ...
    return KcpPlan(
        ...
        scan_chunk_indices=prepare_chunk_indices(scan_cu_cpu, FLA_CHUNK_SIZE).to(
