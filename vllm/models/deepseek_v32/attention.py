...
class DeepseekV32Attention(MLAAttention):
    ...
    def forward(  # type: ignore[override]
        ...
        if attn_metadata is None or (
            self.use_pcp and self._fused_pcp_norm_rope is None
        ):
            mla_kv_cache = None
            mla_k_scale = None
            indexer_k_cache = None
    ...
    def _sparse_indexer_and_attn(
        ...
        use_portable_pcp = self.use_pcp and self._fused_pcp_norm_rope is None
        if self.indexer is not None and not self.skip_topk:
            ...
            sparse_attn_indexer(
                ...
                self.topk_indices_buffer,
                skip_k_cache_insert=not use_portable_pcp,
