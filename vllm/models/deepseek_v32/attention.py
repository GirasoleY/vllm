...
class DeepseekV32Attention(MLAAttention):
    ...
    def __init__(
        ...
        self._fused_pcp_norm_rope: DirectPCPFusedNormRopeWorkspace | None = (
            get_fused_pcp_norm_rope_workspace(
                vllm_config,
                config,
                self.kv_cache_dtype,
        ...
        if self.indexer is not None:
            ...
            self.indexer.indexer_op.skip_k_cache_insert = (
                not self.use_pcp or self._fused_pcp_norm_rope is not None
            )
    ...
    def forward(  # type: ignore[override]
        ...
        if has_indexer and self.use_pcp and self._fused_pcp_norm_rope is None:
            assert index_k is not None
            index_k_out = torch.empty_like(index_k)

        use_fused_pcp_norm_rope = (
            self._fused_pcp_norm_rope is not None and attn_metadata is not None
        )
        if attn_metadata is None or (self.use_pcp and not use_fused_pcp_norm_rope):
            mla_kv_cache = None
            mla_k_scale = None
            indexer_k_cache = None
            mla_slot = None
    ...
    def _sparse_indexer_and_attn(
        ...
        if self.indexer is not None and not self.skip_topk:
            ...
            if self.use_pcp and self._fused_pcp_norm_rope is None:
                assert index_k is not None
