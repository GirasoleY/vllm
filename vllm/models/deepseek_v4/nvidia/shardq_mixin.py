...
class ShardQMixin(_ShardQMixinBase):
    ...
    def _build_shardq_prefill_indexer_metadata(
        ...
        chunks = [
            DeepseekV32IndexerPrefillChunkMetadata(
                ...
                block_table=block_table[cl["req_idx"] : cl["req_idx"] + 1],
    ...
    def _build_shardq_indexer_layout(
        ...
        """
        segment_req_ids_cpu = split.segment_req_ids.detach().cpu().tolist()
        segment_qsl_cpu = split.local_query_start_loc.detach().cpu().tolist()
        ...
        local_seq_lens_cpu = torch.tensor(
            local_seq_lens.detach().cpu().tolist(), dtype=torch.int32
        )
        ...
        chunk_specs: list[tuple[slice, slice]] = []
        for req_idx in range(len(ordered_req_ids)):
            chunk_specs.extend(
                split_indexer_prefill_chunks(
                    compressed_seq_lens_cpu[req_idx : req_idx + 1],
                    query_lens_cpu[req_idx : req_idx + 1],
                    max_prefill_buffer_size,
                    max_logits_bytes,
        ...
        for req_slice, query_slice in chunk_specs:
            req_idx = req_slice.start
            if req_idx is None or req_slice.stop != req_idx + 1:
                raise ValueError("shardq local indexer chunks must contain one request")
            if query_slice.start is None or query_slice.stop is None:
                raise ValueError("shardq local indexer query slice is incomplete")

            total_seq_lens = int(compressed_seq_lens_cpu[req_idx].item())
            if total_seq_lens == 0:
                continue

            cu_seq_lens = torch.empty(2, dtype=torch.int32, device=device)
            cu_seq_lens[0] = 0
            cu_seq_lens[1] = compressed_seq_lens[req_idx]
            token_to_seq = torch.zeros(total_seq_lens, dtype=torch.int32, device=device)

            req_query_start = int(local_qsl_cpu[req_idx].item())
            token_start = req_query_start + query_slice.start
            token_end = req_query_start + query_slice.stop
            cu_seq_len_ks = torch.zeros(
                token_end - token_start, dtype=torch.int32, device=device
            )
            cu_seq_len_ke = (
                coalesced_positions[token_start:token_end] + 1
            ) // indexer_compress_ratio
            cu_seq_len_ke = torch.minimum(
                cu_seq_len_ke,
                compressed_seq_lens[req_idx].expand_as(cu_seq_len_ke),
            )
    ...
    def _build_shardq_prefill_plan(
        ...
        return {
            ...
            "seq_lens_cpu": local_seq_lens.detach().cpu().tolist(),
            "gather_lens_cpu": gather_lens.detach().cpu().tolist(),
            "local_qsl_cpu": local_query_start_loc.detach().cpu().tolist(),
