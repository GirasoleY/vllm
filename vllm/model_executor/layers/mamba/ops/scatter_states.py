...
def _scatter_states_kernel(
    ...
    if CONV_DIM > 0:
        if block_idx * BLOCK_SIZE < CONV_DIM:
            ...
            conv_mask = (offsets < CONV_DIM)[:, None] & (columns < CONV_WIDTH)[None, :]
            # Load the entire old window before writing its shifted successor.
            prefix = tl.load(
                conv_base + (tail_indices + CONV_WIDTH)[None, :] * stride_conv_token,
...
def scatter_states(
    ...
    *,
    ...
    Optional convolution inputs publish the final raw-input window whose width
    is given by ``conv_tail_indices``. Negative selectors address old cache
    columns, zeroed for fresh sequences. Extra cache columns stay unchanged.
    """
    assert state.ndim >= 2
    ...
    if conv_state is not None:
        ...
        assert conv_tail_indices.ndim == 2
        conv_width = conv_tail_indices.shape[1]
        assert conv_width > 0
        assert conv_state.ndim == 3 and conv_state.shape[-1] >= conv_width
        ...
        assert conv_tail_indices.shape[0] == indices.numel()
